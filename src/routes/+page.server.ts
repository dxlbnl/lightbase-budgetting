...
const budgetAmounts = db.$with('budgetAmounts').as(
	db
		.select({
			id: budgetsTable.id,
...
const getValidBudgetsFor = (team: string) =>
	db.with(budgetAmounts).select().from(budgetAmounts).where(and(eq(budgetAmounts.team, team), gt(budgetAmounts.remaining, 0)))
